...
function New-MD5Hashes {
    ...
    $filesToProcess = Get-ChildItem -Path $baseLocationPath -File -Recurse -Force | Where-Object {
        $_.FullName -notlike "*$checksDir*" -and $_.Name -ne $currentScriptFileName
    }

    if ($filesToProcess.Count -eq 0) {
        Write-Host "No files found to process (excluding '$currentScriptFileName' and items in '$checksDir')."
        return
    }
...
function Test-MD5Hashes {
    ...
    if ($filesToProcess.Count -eq 0) {
        Write-Host "No files found on disk to verify (excluding script and checks directory)."
        return
    }
    ...
    foreach ($diskRelPath in $diskFiles.Keys) {
        ...
        if ($checksumDB.ContainsKey($diskContentHash)) {
            ...
            if ($pathEntry) {
                ...
                [void]$processedDBPaths.Add("$($diskContentHash):$($diskRelPath)") # FIX: Suppress boolean output from .Add()
            } else {
    ...
    if ($movedCount -gt 0) {
        Write-Host "MOVED (known content found at a new/unexpected location): $movedCount" -ForegroundColor Blue
    ...
    if ($deletedCount -gt 0) {
        Write-Host "DELETED (file no longer at a previously known location for its content): $deletedCount" -ForegroundColor Red
    ...
    if ($modifiedCount + $movedCount + $newCount + $deletedCount -eq 0) {
        ...
        if ($movedCount + $newCount > 0) {
             Write-Host "Consider running 'New-MD5Hashes' to update the database with new/moved files." -ForegroundColor DarkCyan
        }
    }
}

# --- Main CLI Menu (no changes from your version) ---
function Show-Menu {
    Write-Host "--------------------------------"
